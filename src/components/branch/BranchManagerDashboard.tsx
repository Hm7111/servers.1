...
interface BranchState {
  activeTab: 'dashboard' | 'registrations' | 'members' | 'requests' | 'employees';
  branchData: any;
  stats: {
...
const BranchManagerDashboard: React.FC = () => {
  ...
  const loadBranchData = async () => {
    try {
      ...
      if (result.success) {
        ...
        console.error('Failed to load branch data:', result.error);
  ...
  const navigationItems = [
    ...
    },
    {
      ...
      badge: state.stats.pendingRegistrations > 0 ? state.stats.pendingRegistrations.toString() : undefined,
      color: 'blue'
    },
    {
      id: 'requests',
      ...
      badge: state.stats.activeRequests > 0 ? state.stats.activeRequests.toString() : undefined,
      color: 'amber'
    },
    {
      id: 'members',
      ...
      badge: state.stats.totalMembers > 0 ? state.stats.totalMembers.toString() : undefined,
      color: 'purple'
    },
    {
      id: 'employees',
      ...
      badge: state.stats.employeesCount > 0 ? state.stats.employeesCount.toString() : undefined,
      color: 'indigo'
    }
  ];
  ...
  const renderContent = () => {
    switch (state.activeTab) {
      case 'dashboard':
        return <BranchDashboard stats={state.stats} branch={state.branchData} isLoading={state.isLoading} onRefresh={loadBranchData} recentActivities={recentActivities} />;
      case 'registrations':
